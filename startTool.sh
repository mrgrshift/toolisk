...
source configtoolisk.sh
	...
	echo "$TIME blockheight.sh started. You can view the process here: screen -r blockheight" >> /home/$USERTOOL/toolisk/startTool.log
        screen -dmS consensus bash -c '/home/$USERTOOL/toolisk/consensus.sh; exec bash'
        echo "$TIME consensus.sh started. You can view the process here: screen -r consensus" >> /home/$USERTOOL/toolisk/startTool.log

	#email alert
	startup_log=$(cat /home/$USERTOOL/toolisk/startup.log)
	start_log=$(cat /home/$USERTOOL/toolisk/startTool.log)
